...
function upgradeAdmonitions() {
  for (const blockquote of document.querySelectorAll(".markdown-body blockquote")) {
    ...
    callout.className = `cockpit-microshift-callout is-${ALERT_TYPES[label]}`;

    const title = document.createElement("span");
    title.className = "cockpit-microshift-callout-title";
...
function buildPageToc() {
  const tocRoot = document.getElementById("cockpit-microshift-toc-list");
  const tocWrapper = document.getElementById("cockpit-microshift-page-toc");
